/**
 * WebSocket Threaded - Communication temps réel avec support des threads multiples
 * 
 * Permet de gérer plusieurs onglets/conversations simultanées
 * Chaque client a son propre thread actif, même client_id = même session navigateur
 */
...
export class WebSocketThreadedManager {
  ...
  private setupListeners(): void {
    this.wss.on('connection', (ws: WebSocket, req: any) => {
      ...
      // Détermine le thread à utiliser
      let threadId: string;
      if (requestedThreadId) {
        const thread = this.threadManager.getThread(requestedThreadId);
        threadId = thread ? thread.id : this.threadManager.getMainThread()?.id || this.threadManager.createThread('New Thread');
      } else {
        // Crée un nouveau thread par défaut pour ce client
        threadId = this.threadManager.createThread('New Thread');
      }
  ...
  private sendThreadHistory(ws: WebSocket, threadId: string): void {
    const messages = this.threadManager.getThreadMessages(threadId);
    this.sendTo(ws, {
      type: 'history',
      payload: { messages, threadId },
    });
    console.log(`[WebSocket] Historique envoyé pour thread ${threadId}: ${messages.length} messages`);
  }
