...
export function getToolDefinitions(): Tool[] {
  return [
    ...
    {
      name: 'get_balances',
      description: 'Récupère les crédits disponibles sur les comptes API (Claude, Mistral, Kimi). Utile pour vérifier le solde restant avant d\'effectuer des opérations coûteuses.',
      input_schema: {
        type: 'object',
        properties: {
          providers: {
            type: 'array',
            description: 'Liste des providers à vérifier (optionnel - par défaut tous)',
            items: {
              type: 'string',
              enum: ['claude', 'mistral', 'kimi']
            }
          }
        },
        required: []
...
export class ToolExecutor {
  ...
  async execute(toolName: string, input: ToolInput): Promise<ToolResult> {
    ...
    switch (toolName) {
      ...
      case 'get_balances': {
        const providers = (input.providers as string[]) || ['claude', 'mistral', 'kimi'];
        ...
        if (providers.includes('claude')) {
          try {
            const anthropicKey = fs.existsSync(PATHS.ANTHROPIC_KEY_FILE) 
              ? fs.readFileSync(PATHS.ANTHROPIC_KEY_FILE, 'utf-8').trim()
              : '';
            ...
            if (anthropicKey) {
              ...
              if (response.ok) {
                ...
                balances.claude = {
                  available: true,
                  credits_remaining: data.credits_remaining || 'N/A',
                  usage_tier: data.usage_tier || 'N/A'
                };
              } else {
                balances.claude = {
                  available: false,
              ...
              balances.claude = { available: false, error: 'Clé API non configurée' };
            }
          } catch (error) {
            balances.claude = { available: false, error: (error as Error).message };
          }
        }
        ...
        if (providers.includes('mistral')) {
          try {
            ...
            if (mistralKey) {
              const response = await fetch('https://api.mistral.ai/v1/user', {
                headers: {
              ...
              balances.mistral = { available: false, error: 'Clé API non configurée' };
            }
          } catch (error) {
            balances.mistral = { available: false, error: (error as Error).message };
          }
        }
          ...
          try {
            ...
            if (kimiKey) {
              ...
              if (response.ok) {
                ...
                balances.kimi = {
                  available: true,
                  credits_remaining: data.data?.wallet?.balance || 'N/A',
                  currency: 'CNY'
                };
              } else {
                balances.kimi = {
              ...
              balances.kimi = { available: false, error: 'Clé API non configurée' };
            }
          } catch (error) {
            balances.kimi = { available: false, error: (error as Error).message };
          }
        }
